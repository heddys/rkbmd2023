...
class Kadis_model extends CI_Model{
    ...
    public function get_sisa_per_aset($kib,$lokasi) {
        ...
        if($this->session->userdata('role') == "Pengurus Barang Pembantu UPTD" ) {
            $query = $this->db->query("SELECT a.register FROM `2025_v1`.`kib_awal` a where a.extrakomtabel_baru = '' and a.hapus = '' and a.kode108_baru like '".$kib."%' and a.nomor_lokasi_baru IN ( '".implode("','",$lokasi)."' ) and NOT EXISTS (SELECT y.register from `rkbmd2023`.register_isi y where a.register=y.register) union SELECT a.register FROM `2025_v1`.`kib` a where a.extrakomtabel_baru = '' and a.hapus = '' and a.kode108_baru like '".$kib."%' and a.nomor_lokasi_baru IN ( '".implode("','",$lokasi)."' ) and NOT EXISTS (SELECT y.register from `rkbmd2023`.register_isi y where a.register=y.register)");
        } else {
