...
class Auth_model extends CI_Model{	
	...
	function get_tabel_komponen($where){
		$this->db->select('tabel_list_rkb.id,tabel_list_rkb.date,tabel_list_rkb.time,tabel_list_rkb.id_komponen,tabel_list_rkb.saldo_komponen,tabel_list_rkb.saldo_existing_komponen,tabel_list_rkb.saldo_ideal_komponen,tabel_list_rkb.keterangan,tabel_kode_komponen.kode_komponen,tabel_kode_komponen.nama_komponen,tabel_kode_komponen.spek1,tabel_kode_komponen.spek2,tabel_kode_komponen.merek,tabel_kode_komponen.satuan,tabel_kode_komponen.nilai,tabel_kode_komponen.kode_rekening,tabel_kode_komponen.rincian_kode_rek,sum(tabel_list_rkb.saldo_kegiatan) as volume');    
		$this->db->from('tabel_list_rkb');
	...
	function satuan_komp ($where){
		...
		$this->db->select('*');
		$this->db->from('tabel_kode_komponen');
		$this->db->where($where);
		$query = $this->db->get();
		if($query->num_rows() > 0) {
			return $query->row();
	...
	function cek_data($where){
		$this->db->select('tabel_list_rkb.saldo_existing_komponen,tabel_list_rkb.saldo_ideal_komponen,tabel_list_rkb.saldo_komponen,tabel_kode_komponen.satuan,tabel_kode_komponen.tanda');
		$this->db->from('tabel_list_rkb');
		$this->db->join('tabel_kode_komponen','tabel_list_rkb.id_komponen = tabel_kode_komponen.id');
		$this->db->where($where);
		$query=$this->db->get();
