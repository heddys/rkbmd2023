              ...
              <div class="row" style="float:right;">
                ...
                <table>
                  ...
                  <tbody>
                    <tr>
                      <td width="70%">
                        <form role="form" action="<?php echo site_url();?>/home_guest/cari_register/<?php echo $kib_apa?>" method="post">
                          <div class="col-md-5">
                          <select class="form-control select2" id="select_lokasi" name="select_lokasi" required="required">
                              ...
                              <option value="13.30">Semua Lokasi</option>
                                <?php $x=1; foreach ($lokasi->result() as $row) {?>
                                <option value="<?php echo $row->nomor_unit;?>"><?php echo $row->unit;?></option>
                                <?php }?>
                            </select> 
                          </div>
                  ...
                  <tbody>
                    <?php $x=$offset+1; foreach ($register as $row) {?>
	                	<tr>
                            ...
                            <td>
                                <?php if ($row->status==1) {?>
                                    <center><h5><span class="badge badge-warning"> Proses Invetarisasi </span></h5></center>
                                <?php } elseif ($row->status==2 || $row->status==3) { ?>
                                    <center><h5><span class="badge badge-success"> Suda Di Lakukan Invetarisasi </span></h5></center>
                                <?php } else { ?> <center><h5><span class="badge badge-danger"> Belum Di Lakukan Invetarisasi </span></h5></center> <?php } ?>
                            </td>
	                  	</tr>
	                  <?php }?>
	                </tbody>
